...
Description: Lists all active extensions in the current Qwen Code session. See Qwen Code Extensions.
/help (or /?)
...
Note: For more details on how context files contribute to hierarchical memory, see the CLI Configuration documentation.
/restore
...
User Commands (Global): Located in ~/.qwen/commands/. These commands are available in any project you are working on.
Project Commands (Local): Located in <your-project-root>/.qwen/commands/. These commands are specific to the current project and can be checked into version control to be shared with your team.
...
Inject Commands: Use the !{...} syntax in your prompt to specify where the command should be run and its output injected.
Confirm Execution: When you run the command, a dialog will appear listing the shell commands the prompt wants to execute.
